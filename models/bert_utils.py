...
import dependencies.keras_bert.keras_bert.bert as keras_bert
...
#from dependencies.keras_bert.keras_bert.loader import load_trained_model_from_checkpoint
...
def generate_bert_with_pretrained(pretrained_path, nr_classes=4, weights_path=None):
    ...
    model = load_bert(pretrained_path, checkpoint_path=None, weights_path=weights_path, compile_=True)
    #model = keras.models.load_model(pretrained_path, compile=True)
    #print(model['config'])
    #print(model['weights'])
...
def smart_generator_to_dataset(generator, batch_size=None):
    """
    Convierte automáticamente un generador Keras en un tf.data.Dataset, detectando
    las formas, tipos y presencia de etiquetas.

    Args:
        generator: instancia de keras.utils.Sequence o generador tipo __getitem__.
        batch_size: opcional, si el generador no lo maneja internamente.
    ...
    # Asegurar que x sea tupla de entradas
    if isinstance(x, (list, tuple)):
        x = tuple(np.array(i) for i in x)
    else:
        x = (np.array(x),)

    # Detectar output_signature de las entradas
    input_signature = tuple(
        tf.TensorSpec(shape=(None, *i.shape[1:]), dtype=tf.as_dtype(i.dtype)) for i in x
    )

    # Detectar output_signature de la etiqueta
    if with_labels:
        y = np.array(y)
        label_signature = tf.TensorSpec(shape=(None, *y.shape[1:]), dtype=tf.as_dtype(y.dtype))
        output_signature = (input_signature, label_signature)
    else:
        output_signature = input_signature

    # Definir generador Python
    def data_gen():
        for i in range(len(generator)):
            sample = generator[i]
            if with_labels:
                x, y = sample
                x = tuple(np.array(i) for i in x) if isinstance(x, (list, tuple)) else (np.array(x),)
                y = np.array(y)
                yield x, y
            else:
                x = sample
...
def predict(model, test_generator, roc_auc=True, classes=None,
    ...
    print(type(predict_g[1]))      # <class 'tuple'>
    print(len(predict_g[1]))
    all_preds = []
    for i in range(len(predict_g)):
        batch_input = predict_g[i]
        preds = model.predict(batch_input, verbose=0 if nonverbose else 1)
        all_preds.append(preds)

    preds = np.concatenate(all_preds, axis=0)  
    #predict_dataset = smart_generator_to_dataset(predict_g)
    #preds = model.predict(predict_g[0], verbose=0 if nonverbose else 1)
